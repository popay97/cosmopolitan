...
export default async function handler(req, res) {
    ...
    if (type === "price") {
        try {
            missingObj.validFrom = new Date(Date.UTC(missingObj.validFrom));
            missingObj.validTo = new Date(Date.UTC(missingObj.validTo));
        ...
        for (let i = 0; i < resToUpdate.length; i++) {
            ...
            try {
                ...
                if (objForSave.hasLocation) {
                    const pricesIncoming = await PricesModel.findOne({ airport: objForSave.arrivalAirport, destination: objForSave.billingDestination, validFrom: { $lte: new Date(Date.UTC(objForSave.arrivalDate)) }, validTo: { $gte: new Date(Date.UTC(objForSave.arrivalDate)) }, type: 'incoming' }).lean();
                    const pricesOutgoing = await PricesModel.findOne({ airport: objForSave.arrivalAirport, destination: objForSave.billingDestination, validFrom: { $lte: new Date(Date.UTC(objForSave.booked)) }, validTo: { $gte: new Date(Date.UTC(objForSave.booked)) }, type: 'outgoing' }).lean();
                    var pricing = {
                        ways: 0,
                        calculated: false,
                        incomingInvoice: {
