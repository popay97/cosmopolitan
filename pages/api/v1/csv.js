...
export default async function handler(req, res) {
  ...
  for (let i = 0; i < csvData.length; i++) {
    ...
    objForSave = {
      ...
      booked:
        csvData[i][2] && csvData[i][3]
          ? formatDate(csvData[i][2], csvData[i][3])
          : null,
      ...
      arrivalDate:
        csvData[i][19] && csvData[i][20]
          ? formatDate(csvData[i][19], csvData[i][20])
          : null,
      arrivalFlight: {
      ...
      depDate:
        csvData[i][23] && csvData[i][24]
          ? formatDate(csvData[i][23], csvData[i][24])
          : null,
    ...
    if (objForSave.hasLocation) {
      ...
      if (
        arrivalDate.getMonth() === depDate.getMonth() &&
        arrivalDate.getFullYear() === depDate.getFullYear()
      ) {
        pricing.ways = 2;
      } else {
        pricing.ways = 1;
      ...
      if (pricesIncomingArrival && pricesIncomingDep) {
        if (objForSave.transfer === "STR") {
          pricing.incomingInvoice.totalw1 = Number(
            (
              objForSave.adults * pricesIncomingArrival.shared +
              objForSave.children * pricesIncomingArrival.shared * 0.5
            ).toFixed(3)
          );
          pricing.incomingInvoice.totalw2 = Number(
            (
              objForSave.adults * pricesIncomingDep.shared +
              objForSave.children * pricesIncomingDep.shared * 0.5
            ).toFixed(3)
          );
          ...
          if (objForSave.adults + objForSave.children <= 3) {
            ...
            pricing.incomingInvoice.total = Number(
              (
                pricing.incomingInvoice.totalw1 +
                pricing.incomingInvoice.totalw2
              ).toFixed(3)
            );
          } else {
            pricing.incomingInvoice.totalw1 = Number(
      ...
      if (pricesOutgoingArrival && pricesOutgoingDep) {
        if (objForSave.transfer === "STR") {
          ...
          pricing.outgoingInvoice.handlingFee = Number(
            (objForSave.adults * 4.5 + objForSave.children * 2.25).toFixed(3)
          );
          pricing.outgoingInvoice.totalw1 = Number(
            (
              pricesOutgoingArrival.shared * objForSave.adults +
              pricesOutgoingArrival.shared * objForSave.children * 0.5
            ).toFixed(3)
          );
          pricing.outgoingInvoice.totalw2 = Number(
            (
              pricesOutgoingDep.shared * objForSave.adults +
              pricesOutgoingDep.shared * objForSave.children * 0.5
            ).toFixed(3)
          );
          ...
          if (objForSave.adults + objForSave.children <= 3) {
            ...
            pricing.outgoingInvoice.total = Number(
              (
                pricing.outgoingInvoice.totalw1 +
                pricing.outgoingInvoice.totalw2
              ).toFixed(3)
            );
            pricing.outgoingInvoice.totalWithFee =
